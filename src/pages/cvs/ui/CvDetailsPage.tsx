...
export function CvDetailsPage({ cvId, initialTab = "details" }: CvDetailsPageProps) {
  ...
  return <>
    ...
    <section role="tabpanel" id={`cv-panel-${activeTab}`} aria-labelledby={`cv-tab-${activeTab}`} tabIndex={0}
      ...
      {activeTab === "preview" && (loading || error || !cv) && <div className="mb-6 flex justify-end"><Button variant="outline" className="w-button-width border-primary text-primary" disabled>Export PDF</Button></div>}
      {loading ? <Skeleton role="status" aria-label="Loading CV" className="h-64 w-full" />
        ...
        : activeTab === "preview" && account ? <CvPreview key={cv.id} cvId={cv.id} ownerId={account.id} />
        : null}
    </section>
  </>;
}
