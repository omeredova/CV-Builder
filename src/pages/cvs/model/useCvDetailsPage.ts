...
import { cvQuery, cvSkillsQuery, cvHeaderQuery, type Cv, type CvSkills, type CvQueryData, type CvQueryVariables } from "@/entities/cv";
...
export function useCvDetailsPage(cvId: string, initialTab: CvTab) {
  ...
  const header = useQuery<CvQueryData<Pick<Cv, "id" | "name" | "user">>, CvQueryVariables>(cvHeaderQuery, {
    fetchPolicy: "network-only", variables: { cvId }, skip: !account || activeTab === "details" || activeTab === "skills", context: { skipGlobalLoader: true },
  });
  const query = activeTab === "details" ? details : activeTab === "skills" ? skills : header;
  ...
  return { cv, cvName, detailsCv: details.data?.cv, skillsCv: skills.data?.cv, account, activeTab, openTab, loading: query.loading || accountLoading, error: query.error || accountError, retry };
}
