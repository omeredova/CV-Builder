...
export interface SkillsManagementProps {
  ...
  removalActionLabel?: string;
  disableUntilValid?: boolean;
}

export function SkillsManagement({ skills, categories, operations, removalActionLabel = "DELETE", disableUntilValid = false }: SkillsManagementProps) {
  ...
  return <Management
    ...
    form={(state.mode.kind === "add" || state.mode.kind === "update") && <SkillFormDialog
      ...
      assignedSkills={skills} operations={operations} disableUntilValid={disableUntilValid} onSaved={state.reset} onClose={state.reset} />}
    removal={{
      ...
      actionLabel: removalActionLabel, title: "Remove skills",
